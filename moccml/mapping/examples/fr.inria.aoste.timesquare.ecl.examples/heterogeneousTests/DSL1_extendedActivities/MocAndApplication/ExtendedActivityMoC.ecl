...
package uml
	context Class
		def : referenceClock : Event = GenericEvent
...
-- here it is difficult to have a rule.... it depends on the path and the possible fork in it. Consequently we consider here only one final node
		inv lastFinalNodeStopActivity:
			let firstFinalNode = Expression Inf(self.node->select(n|n.oclIsKindOf(FinalNode))->asSequence()->first().oclAsType(FinalNode).finishIt) in
