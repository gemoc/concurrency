...
public class Services {
	...
	public String getVariablestoString (RelationDefinition def){
		EList<PrimitiveElement> lst = new BasicEList<>();
		StringBuilder st= new StringBuilder();
		
		lst = getVariables(def);
		
		for (PrimitiveElement ce : lst) {
			if (st.length()!=0) {
				st.append(" ");
			}
	...
	public String getExpression(IntegerAssignement action){
		...
		{
			result = result.replaceAll("ten", "10");
		}
		
		result.replaceAll(getActionVariableName(action), getActionVariableName(action)+'_');
		return result;
	}
	...
	private String print(ClassicalExpression ce){
		...
		case ClassicalExpressionPackage.INTEGER_REF:
			{
				//String s = filterIntegerNames(((IntegerRef)ce).getIntegerElem().getName());
				//return s;
...
public String toStringAction(Transition t){
	...
	public String filterIntegerNames(String integarName)
	{
		...
		s= "9";
		else if(integarName.equals("ten")==true)
		s= "10";
		else 
			s = integarName;
